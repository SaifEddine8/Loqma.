...
DateTime? selectedDate;
  ...
  List<DropdownMenuItem<String>>addcategories=categories.skip(1).map((category)=>
      ...
      body: Padding(
        ...
        child: Form(
                ...
                InkWell(
                  ...
                  child: Container(
                    ...
                    Column(
                      ...
                      children: [
                        Icon(Icons.add_a_photo_outlined,size: MediaQuery.of(context).size.height/8,color: ConstantColors.primaryColor,),
                        SizedBox(height: 8,),
                        Text(
          ...
          setState(() {
            ...
            productionDateController.text = "${picked.year}/${picked.month}/${picked.day}";
          });
              }
            },
            decoration: InputDecoration(
              ...
              suffixIcon: const Icon(Icons.date_range), // أيقونة الكالندر على الطرف
              border: OutlineInputBorder(
          ...
          setState(() {
            expiryDate = picked;
            // تحديث النص الظاهر داخل الحقل
            expiredDateController.text = "${picked.year}/${picked.month}/${picked.day}";
          });
              }
            },
            decoration: InputDecoration(
          ...
          TextFromFieldClass(controller: descriptionController,hint: 'Product Description',lable: 'description',
          validator: (value) {
            if(value==null||value.isEmpty)
            {
            ...
            return null;
               ...
               Material(
                      ...
                      child: Column(
                        children: [
                          ...
                          Form(child: Column(
                            spacing: 10,
                            children: [
                              TextFromFieldClass(controller: originPrice,hint: 'Origin Price',preIcon: Icons.attach_money,validator: (value){
                              ...
                              ,),
                              TextFromFieldClass(controller: discount,hint: 'Discount',help: '30 <= discount <= 100',preIcon: Icons.percent,validator: (value){
                                if(value!.trim().isEmpty)
                                {
                                  return 'required';
                                }
                          ...
                          onChanged: () => setState(() {
                            final Discount = double.tryParse(discount.text.trim());
                            final price = double.tryParse(originPrice.text.trim());
                            if (Discount != null &&
                                Discount >= 30 &&
                                Discount < 100 &&
                                price != null &&
                          ...
                          Column(
                            children: [
                              SizedBox(height: 10,),
                              Text('the final price after discount : ',style: ConstantStyle.priceStyle,),
                              Text('${finalPrice} JD',style: ConstantStyle.priceStyle,)
                  ...
                  ElevatedButton(onPressed: (){
                    ...
                    final diferenceDay=expiryDate!.difference(productionDate!).inDays;
                    final maxAllowedDays=categoryMaxDays[selectedCategory];
                    if(diferenceDay>maxAllowedDays!)
                    {
                       ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('the diference detween dates greater than the limit is allowed'),
                        ),
                      );
                      return;
                    }
                    final offer=Offer(title:nameController.text , description: descriptionController.text, quantity: int .parse(quantityController.text), expiryDate: expiryDate!, productionDate: productionDate!, image: _selectedImage!.path, category: selectedCategory!,type: selectedType!,originalPrice: selectedType==OfferType.sale? double.parse(originPrice.text):null,price: selectedType == OfferType.sale
                  ? finalPrice
                  : 0, 
